            ...
            jQuery(x).on("mousedown", function (e) {
              ...
              jQuery(x).on("touchstart", function (e) {
                ...
                const moveHandler = function (e) {
                  const r = e.touches[0].clientX;
                  const n = jQuery(t).offset().left;
                  const i = jQuery(t).width();
                  const s = jQuery(x).css("right");
                  const c = (((n + i - r) / i) * 100).toFixed(2);
                  const a = c > 100 ? 100 : c < 0 ? 0 : c;

                  jQuery(j).width(a + "%");
                  jQuery(x).css("right", a + "%");

                  if (s !== jQuery(x).css("right")) {
                    void 0 !== o.onMoving && o.onMoving();
                  }
                };

                jQuery(document).on("touchmove", moveHandler);

                jQuery(document).on("touchend touchcancel", function () {
                  jQuery(document).off("touchmove", moveHandler);
                  k = !0;
                  jQuery(x).css("opacity", o.opacity);
                  void 0 !== o.onMoveEnd && o.onMoveEnd();
                });
